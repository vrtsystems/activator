	...
	async.waterfall([
		...
		function (res,cb) {
			if (!res || res.length < 1) {
				...
				mailer("passwordreset",req.lang||"en_US",
					{code:reset_code,authorization:reset_code,email:email,id:id,request:req},from,email,attachments.passwordreset,cb);
			}
		}
	],function (err) {
		var code = 400;
		...
		function (res,cb) {
			if (!res) {
				...
				try {
                    ...
                    var email = getObjectProperty(res, emailProperty);
                    mailer("newpassword",req.lang||"en_US",
                        {request:req},from,email,attachments.passwordreset,cb);
				} catch (e) {
					cb(e);
				}
			}
