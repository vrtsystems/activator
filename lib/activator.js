...
completeActivate = function (req, done, addonFunctions) {
			...
			if (!res) {
				...
				try {
					...
					if (decoded.purpose !== "activation" ||
						decoded.sub !== getObjectProperty(res, emailProperty)) {
						throw new Error("invalid code");
					}

					if (addonFunctions) {
						// Do additional validation and other misc functions
						for (let index = 0; index < addonFunctions.length; index++) {
							let addonFunction = addonFunctions[index];
							let error = addonFunction(req, /*find model=*/res);
							if (error) {
								throw error;
							}
						}
						...
						return model.activate(res,password,cb);
					}
					else {
						// For backward compatibility
						// Password would have already set upon init activate
						// Only set the status to activate from here on.
						model.activate(idProperty?getObjectProperty(res, idProperty):id,password,cb);
					}
				} catch (e) {
					return cb(e);
				}
...
module.exports = {
	...
	completeActivate: function (req,res,next,addonFunctions) {
		rparam(req);
		completeActivate(req,function (code,message) {
			res.status(code).send(message);
		}, addonFunctions);
	},
	completeActivateNext: function (req,res,next,addonFunctions) {
		rparam(req);
		completeActivate(req,function (code,message) {
			req.activator = req.activator || {};
			_.extend(req.activator,{code:code,message:message});
