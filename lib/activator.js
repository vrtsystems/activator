		...
		function (res,cb) {
			if (!res || res.length < 1) {
				...
				mailer("passwordreset",req.lang||"en_US",
					{code:reset_code,authorization:reset_code,email:email,id:id,request:req,
                        wideskyTempPassword:req.param("wideskyTempPassword")},from,email,attachments.passwordreset,cb);
			}
		}
	],function (err) {
		var code = 400;
